...
class maisong{
  ...
  constructor(object:JSON){
    ...
    this.song_ds_summary=(<Array<number>><any>object["ds"]).join("/")
...
class maimai_song_list{
  ...
  constructor(ctx:Context){
    ctx.http("GET","https://www.diving-fish.com/api/maimaidxprober/music_data").then(
      (response) =>{
...
export function apply(ctx: Context,config:Config) {
  ...
  ctx.command("maimai <id:number> [diff:string] 根据id或难度查询乐曲或谱面信息。")
    .action((argv,id,diff) => {
      ...
      if(song==undefined){
        ...
        chart.get_probe_data(ctx).then(()=>{argv.session.send([song.song_info_summary,
  ...
  ctx.command("maimai")
    ...
    .shortcut(
      /^随个(?:((?:[^写代]*?)(?!写|代))的)?(?:([^代的]*?)写的)?(?:([^写的])代的)?(标准|DX)?(绿|黄|红|紫|白)?([0-9\.+歌]*)$/,
      {options:{artist:"$1",charter:"$2",version:"$3",type:"$4",difficulty:"$5",level:"$6"}})
    .shortcut(
      /^随个(?:([^代的]*?)写的)?(?:((?:[^写代]*?)(?!写|代))的)?(?:([^写的])代的)?(标准|DX)?(绿|黄|红|紫|白)?([0-9\.+歌]*)$/,
      {options:{artist:"$1",charter:"$2",version:"$3",type:"$4",difficulty:"$5",level:"$6"}})
