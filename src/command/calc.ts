...
export default function cmd_calc(ctx: Context, config: Config) {
  function calculate_factor(ach: number): [number, string] {
    ...
    if (ach < 60) return [5, 'C']
    if (ach < 70) return [6, 'B']
    if (ach < 75) return [7, 'BB']
    if (ach < 80) return [7.5, 'BBB']
    if (ach < 90) return [8, 'A']
    if (ach < 94) return [9, 'AA']
    if (ach < 97) return [10.5, 'AAA']
    if (ach < 98) return [12.5, 'S']
    if (ach < 99) return [12.75, 'S+']
    if (ach < 99.5) return [13, 'SS']
    if (ach < 100) return [13.25, 'SS+']
    if (ach < 100.5) return [13.5, 'SSS']
    return [14, 'SSS+']
  }
  ...
  ctx.command('maimai')
    ...
    .action((_, id, diff, achieve) => {
      ...
      const available_great = (100 * 0.2) / tap_basic_num

      return [
        `${song.song_info_summary} ${chart.base_summary}`,
        `物量：${max_combo}，基础计算物量：${tap_basic_num}`,
        `1个 Tap Great -${available_great.toFixed(4)}%`,
        `Break 50 落相当于${((0.5 / (2 * BREaK)) / available_great).toFixed(4)}个 Tap Great (-${(0.5 / (2 * BREaK)).toFixed(4)}%)`,
        `${achieve}% 允许的最大 Tap Great 数量： ${(available / available_great).toFixed(2)}`,
  ...
  ctx.command('maimai')
    ...
    .action((_, base, ach) => {
      ...
      const res = calculate_factor(ach)
      const rating = Math.floor((base * res[0] * Math.min(ach, 100.5)) / 100)
      return [
        `定数： ${base}`,
        `达成率： ${ach}% ${res[1]}`,
        `Rating = ${base}*${res[0]}*(${Math.min(ach, 100.5)}/100) = ${rating}`,
      ].join('\n')
    })
}
