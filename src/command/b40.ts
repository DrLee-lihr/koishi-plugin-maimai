...
export async function query_player(session: Session, username: string, ctx: Context): Promise<player_data> {
  ...
  return ctx.http.post('https://www.diving-fish.com/api/maimaidxprober/query/player', data)
}
...
export default function cmd_b40(ctx: Context, config: Config) {
    ...
    let base = sharp(
      fileSystem.existsSync(cachePath)
        ...
        : await ctx.http('GET', `https://www.diving-fish.com/covers/${id}.jpg`, { responseType: 'arraybuffer' })
          .then((buffer) => {
            (async () => { fileSystem.writeFileSync(cachePath, buffer) })()
            return buffer
          })
          .catch(() => fileSystem.readFileSync(path.resolve(maimai_resource_path, 'no_image.png'))),
    )
  ...
  ctx.command('maimai')
    ...
    .action(async ({ session }, username) => {
      ...
      return query_player(session, username, ctx)
        .then(async (result: player_data) => {
          const background = sharp(`${maimai_resource_path}\\b40.png`)

          const composite_list = [
            {
              input: (await text2svgbuffer(`${result.rating.toString().padStart(4, ' ')}+${
            ...
            {
              input: (await text2svgbuffer(result.nickname ?? result.username, 80, true, false, 'black')),
              left: 400,
              top: 100,
            },
          ]
