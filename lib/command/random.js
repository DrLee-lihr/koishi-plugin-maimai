...
const mai_tool_1 = require("../mai_tool");
function default_1(ctx, config, maisonglist) {
    ctx.command("maimai")
        ...
        .action(({ options }) => {
        var result = maisonglist.filt_chart((chart) => ((options.level != "歌") ? ((0, mai_tool_1.in_level)(chart.ds, options.level)) : true) &&
            ...
            (options.difficulty == "" ? true : (chart.difficulty == mai_tool_1.difficulty_trans_table[options.difficulty])) &&
            (options.type != "" ? ((chart.song.is_sd ? "标准" : "DX") == options.type) : true) &&
            (options.version != "" ? (chart.song.object["basic_info"]["from"] == mai_tool_1.version_transform_table[options.version]) : true));
        ...
        .shortcut(/^随个(?:([^写代0-9.+歌]*?)的)?(?:([^代的0-9.+歌]*?)写的)?(?:([^写的0-9.+歌])代的)?(标准|DX)?(绿|黄|红|紫|白)?([0-9\.+歌]{1,4})(?:[给要当].*)?$/, { options: { artist: "$1", charter: "$2", version: "$3", type: "$4", difficulty: "$5", level: "$6" } })
        .shortcut(/^随个(?:([^代的0-9.+歌]*?)写的)?(?:([^写代0-9.+歌]*?)的)?(?:([^写的0-9.+歌])代的)?(标准|DX)?(绿|黄|红|紫|白)?([0-9\.+歌]{1,4})(?:[给要当].*)?$/, { options: { artist: "$1", charter: "$2", version: "$3", type: "$4", difficulty: "$5", level: "$6" } })
    ...
    ctx.command("maimai")
        ...
        .option("version", "-v [version:string] 谱面版本。", { fallback: "" })
        .action(({ options }) => {
        var result = maisonglist.filt((song) => (song.object["basic_info"]["artist"].toLowerCase().includes(options.artist.toLowerCase())) &&
            (options.version != "" ? (song.object["basic_info"]["from"] == mai_tool_1.version_transform_table[options.version]) : true));
