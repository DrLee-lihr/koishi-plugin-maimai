...
function default_1(ctx, config, maisonglist) {
    ctx.command("maimai <id:number> [diff:string] 根据id或难度查询乐曲或谱面信息。")
        ...
        if (song == undefined) {
            ...
            var diffid = 5;
            for (var i = 0; i <= 4; i++) {
                for (var j = 0; j < mai_tool_1.difficulty_id[i].length; j++) {
                    if (diff == mai_tool_1.difficulty_id[i][j]) {
                        diffid = mai_tool_1.difficulty_id[i][0];
                        break;
                    }
                }
            }
    ...
    ctx.command("maimai")
        ...
        return (0, mai_tool_1.page_split)(temp, config, options.page);
    });
    ctx.command("maimai")
    ...
    ctx.command("maimai")
        ...
        if (bpm2 == undefined) {
            ...
            return (0, mai_tool_1.page_split)(result, config);
        }
        else {
            var result = [];
    ...
    ctx.command("maimai")
        ...
        if (music_cmd == undefined)
            return "未安装此功能的依赖插件 koishi-plugin-music，请确保此插件已安装且被正确加载。";
        else
            return music_cmd.execute({ args: [maisonglist.id(id).object["basic_info"]["title"]], options: { platform: "netease" } });
    });
}
